...
def main():
    ...
    if any(e.get('high_value') for e in evs) and not selftest:  # TOWER-FIX-24-qfa(beat-93):互激阻尼——pair-wake仅高值件才唤;落账致头迁=恒有evs(qlv.beat类低值)致互唤永动环(与qlv塔pair-wake互激成storm,02:39-03:07Z无拍成实测),阻尼后低值不唤
        try:
